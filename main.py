...
def check_url(message):
    ...
    response_message = (
    "<b>🔍 Gateways Fetched Successfully ✅</b>\n"
    "━━━━━━━━━━━━━━\n"
    f"🔹 URL: <code>{url}</code>\n"
...
if not valid_api_key:  # Assuming this is the condition you're checking
...
def generate_cards(message):
    ...
    if bin_info_response.status_code == 200:
        ...
        for card_number in card_numbers:
            ...
            year = random.randint(current_year, current_year + 5)  # Random year within the next 5 years
            cvv = random.randint(100, 999)  # Generate a random 3-digit CVV
            card_numbers_formatted.append(f'''<code>{card_number}|{month:02d}|{year}|{cvv}</code>''')

        card_info += '\n'.join(card_numbers_formatted) + '\n\n'  # Add line space before additional info
        card_info += (
            "𝗜𝗻𝗳𝗼: \n"
...
def welcome(message):
    welcome_text = (
        ...
        "/check <CHECK> - Check details about the specified URL\n"
